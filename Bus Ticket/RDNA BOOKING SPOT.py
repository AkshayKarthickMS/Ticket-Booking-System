...
if b==1:
    ...
    myobj = datetime.now()
    # Replace 'Your_Friend's_Phone_Number' with your friend's phone number in international format (with the country code).
    friend_phone_number = '+91'+mobileno
    # Replace 'Your_Message_Here' with the message you want to send.
    message = x
    # Replace the time in 24-hour format (hh, mm).
    scheduled_time_hour = myobj.hour
    scheduled_time_minute = myobj.minute + 1

    try:
    # Send the WhatsApp message
        kit.sendwhatmsg(friend_phone_number, message, scheduled_time_hour, scheduled_time_minute)

        print(f"Message will be sent to {friend_phone_number} at {scheduled_time_hour}:{scheduled_time_minute}")
